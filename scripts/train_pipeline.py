...
def perform_bayesian_optimization(
    ...
    def objective_wrapper(**params):
        ...
        objective_auroc, val_aucs, auroc_ci, all_preds, all_labels, _, _ = (
            objective_func(
                model_name,
                model_type,
                params,
...
def train_model(
    ...
    if model_type == "ml":
        ...
        X_train = train_df[train_df[label] != -1].drop(
            labels_list + ["PatientID", "Trt_Date"], axis=1
        )
        y_train = train_df[train_df[label] != -1][label]

        X_tune = tune_df[tune_df[label] != -1].drop(
            labels_list + ["PatientID", "Trt_Date"], axis=1
        )
        y_tune = tune_df[tune_df[label] != -1][label]

        X_test = test_df[test_df[label] != -1].drop(
            labels_list + ["PatientID", "Trt_Date"], axis=1
        )
    ...
    if model_type == "dl":
        ...
        tune_loader = DataLoader(
            tune_dataset, batch_size=64, shuffle=False, collate_fn=pad_collate
        )
        test_loader = DataLoader(
            test_dataset, batch_size=test_batch, shuffle=False, collate_fn=pad_collate
        )

        # input size = size - (features that starts with Label) - 2 (PatientID and Trt_Date column)
        input_size = train_df.shape[1] - (
            len([s for s in full_colmuns_list if s.startswith("Label")]) + 2
        )
        ...
        if model_name == "mlp":
            ...
            model = Transformer(
                input_size,
                output_size,
                hidden_size=hidden_size[int(params["hidden_size"])],
                num_heads=num_heads[int(params["num_heads"])],
                num_layers=int(params["num_layers"]),
                dim_feedforward=dim_feedforward[int(params["dim_feedforward"])],
                dropout=params["dropout"],
            ).to(device)
        ...
        if output_size == 1:
            ...
            objective_auroc = val_aucs[0][
                best_epoch
            ]  # find the epoch with the highest AUROC
            ...
            best_epoch = np.argmax(
                mean_aucs
            )  # find the epoch with the highest mean AUROC
...
def main(
    ...
    labels_list = [
        s
        for s in train_df.columns.tolist()
        if s.startswith("Label") and s.endswith(target_change)
    ]
    ...
    for label in labels_list:
        ...
        for model in ml_models:
            print(
                f"Optimizing hyperparameters for Model: {model}, Label: {label} using Bayesian Optimization..."
            )
        ...
        for model in dl_models:
            ...
            optimizer, val_aucs, auroc_ci, all_preds, all_labels = (
            ...
            mean_aucs = [
                np.mean([val_aucs[i][epoch] for i in range(len(val_aucs))])
                for epoch in range(len(val_aucs[0]))
            ]
            best_epoch = np.argmax(
                mean_aucs
        ...
        for model in ml_models:
            objective_auroc, _, _, all_preds, all_labels, best_model, best_ir_model = (
                train_model(
                    model,
                    "ml",
                    search_results[(model, label)]["params"],
                    train_df,
                    tune_df,
                    test_df,
                    label,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Bayesian optimization and model training/testing."
    )
    parser.add_argument(
        "--train_path",
        type=str,
        default=data_path_dict["train"],
        help="Path to the pre-processed training set CSV",
    )
    parser.add_argument(
        "--tune_path",
        type=str,
        default=data_path_dict["tune"],
        help="Path to the pre-processed validation set CSV",
    )
    parser.add_argument(
        "--test_path",
        type=str,
        default=data_path_dict["test"],
        help="Path to the pre-processed test set CSV",
    )
    parser.add_argument(
        "--full_path",
        type=str,
        default=data_path_dict["full"],
        help="Path to the pre-processed full dataset CSV",
    )
    parser.add_argument(
        "--search_save_path",
        type=str,
        default=data_path_dict["save_bayopt_search"],
        help="Save path for bayopt search results",
    )
    parser.add_argument(
        "--test_save_path",
        type=str,
        default=data_path_dict["save_bayopt_test"],
        help="Save path for bayopt test results",
    )
    parser.add_argument(
        "--target_change", type=str, default="3pt_change", help="Target change"
    )
