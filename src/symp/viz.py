...
def plot_repeated_corr_matrix(
    df, labels_list, label_mapping, legend_order, subsample_size=None
):
        ...
        for target2 in labels_list[i + 1 :]:
            ...
            if subsample_size and corr_df["PatientID"].nunique() > subsample_size:
                ...
                sampled_patients = np.random.choice(
                    unique_patients, size=subsample_size, replace=False
                )
            ...
            if len(corr_df) >= 3:
                try:
                    result = pg.rm_corr(
                        data=corr_df, x=target1, y=target2, subject="PatientID"
                    )
...
def plot_performance_curves(metrics, labels_list, label_mapping, legend_order, colors):
    ...
    for i, label in enumerate(ordered_labels):
        ...
        plt.plot(
            ...
            tpr,
            color=colors[i],
            label=f'{label_mapping[label]} (AUC = {metrics["val_auc"][labels_list.index(label)]:.2f})',
        )
    ...
    for i, label in enumerate(ordered_labels):
        ...
        plt.plot(
            ...
            precision,
            color=colors[i],
            label=f'{label_mapping[label]} (AUP = {metrics["val_aup"][labels_list.index(label)]:.2f})',
        )
...
def plot_calibration_curves(
    calibration_curves, labels_list, label_mapping, legend_order, colors
):
    plt.figure(figsize=(6, 6), facecolor="white")
    ...
    for i, label in enumerate(ordered_labels):
        ...
        if i == 0:
            ...
            legend_label = (
                f"{label_mapping[label]} | MCE: {mce:.2f}, MaxCE: {max_ce:.2f}"
            )
        else:
    ...
    plt.plot(
        [0, 1], [0, 1], "k:", label="Perfectly calibrated"
    )  # Adjusted range for calibration line
...
def plot_ecdf(merge_df, label_mapping, legend_order, colors):
    ...
    label_mapping_with_suffix = {
        key + "_prediction": value for key, value in label_mapping.items()
    }

    # Identify columns that end with '_prediction'
    prediction_columns = [
        col for col in merge_df.columns if col.endswith("_prediction")
    ]
    ...
    ordered_predictions = [
        label
        for name in legend_order
        for label in prediction_columns
        if label_mapping_with_suffix.get(label, label) == name
    ]
...
def plot_net_benefit_curves(
    net_benefit_df_list, labels_list, label_mapping, legend_order, colors
):
    ordered_labels = [
        label
        for name in legend_order
        for label in labels_list
        if label_mapping.get(label) == name
    ]
    ...
    for i, label in enumerate(ordered_labels):
        ...
        ax.plot(
            thresh, np.zeros(thresh.shape), label="None", color="black", linestyle="--"
        )
    ...
    narrowest_interval = min(
        all_target_intervals.items(), key=lambda x: x[1][1] - x[1][0]
    )
...
def generate_shap_plots(shap_dict, labels_list, save_path):
    ...
    for label in labels_list:
        ...
        # Layered Violin Plot
        shap.summary_plot(
            top_shap_values, data[top_features], plot_type="layered_violin", show=False
        )
        ...
        # Bar Plot
        filtered_explainer = shap.Explanation(
        ...
        plt.gca().set_xticklabels(
            ["{:.3f}".format(x) for x in plt.gca().get_xticks()], rotation=45
        )
...
def plot_mean_shap_bar(mean_shap_df, colors, top_n=10, sort_by="mean"):
    ...
    sorted_df = mean_shap_df.sort_values(by=sort_by, axis=1, ascending=False).iloc[
        :, :top_n
    ]

    # Set up the plot
    plt.figure(
        figsize=(10, top_n * 0.5)
...
def plot_mean_abs_shap_values(mean_shap_df, temporal_cohort_sheet):
    ...
    transposed_df = pd.merge(
        mean_shap_df.T.abs(),
        temporal_cohort_sheet,
        left_index=True,
        right_on="Features (converted)",
    )
    ...
    # Plotting
    plt.figure(
        figsize=(10, len(grouped_shap_means_abs_sorted) * 0.5)
...
def create_forest_plot(df, colors):
    ...
    ax.errorbar(
        df["OddsRatio"],
        range(len(df)),
        xerr=[df["OddsRatio"] - df["CI.Lower"], df["CI.Upper"] - df["OddsRatio"]],
        fmt="o",
        color="black",
        ecolor=colors,
        elinewidth=3,
    ...
    # Add X-axis title
    ax.set_xlabel(
        "Odds Ratio for Emergency Department Visits", fontsize=16, labelpad=20
    )
    ...
    p_value_position = (
        odds_ratio_position + 0.26
    )  # Adjust spacing to ensure equal length
    ...
    for i, (symptom, row) in enumerate(df.iterrows()):
        ...
        ax.text(
            odds_ratio_position,
            i,
            f"{row['OddsRatio']:.2f} ({row['CI.Lower']:.2f}-{row['CI.Upper']:.2f})",
            ha="left",
            va="center",
            transform=ax.get_yaxis_transform(),
            fontsize=16,
    ...
    header_y_position = (
        len(df) - 9.8
    )  # Incremented by 0.5 to ensure it's above all text entries
    ...
    for idx, df in enumerate(dfs):
        ...
        legend_handles = [
            mpatches.FancyArrowPatch(
                (0, 0), (0.1 * 0.8, 0), color=color, arrowstyle="-|>", mutation_scale=20
            )
            for color in colors
        ]
...
def plot_radar_chart(
    ...
    plt.yticks(
        np.arange(start_from, end_at, scale),
        ["{:.1f}".format(x) for x in np.arange(start_from, end_at, scale)],
        color="grey",
        size=7,
    )
