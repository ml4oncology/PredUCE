...
class EHRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        patient_features = patient_data.drop(columns=['PatientID', 'Trt_Date']+ self.full_label_list).values
...
def pad_collate(batch):
    ...
    for features, labels in batch:
        ...
        padded_features = torch.cat([features, torch.tensor([[-1e6] * features.shape[1]] * pad_len, dtype=torch.float)])
        padded_labels = torch.cat([labels, torch.tensor([[-1] * labels.shape[1]] * pad_len, dtype=torch.float)])
...
def masked_binary_cross_entropy_with_logits(logits, targets, mask, neg_to_pos_ratios):
    ...
    for i in range(len(neg_to_pos_ratios)):
        ...
        loss = nn.functional.binary_cross_entropy_with_logits(
            filtered_logits, filtered_targets, reduction='none', pos_weight=torch.tensor(neg_to_pos_ratios[i])
        )
        ...
        for i, batch in enumerate(dataloader):
            ...
            for j in range(output_size):
                ...
                if current_mask.sum().item() > 0:
                    task_loss = nn.functional.binary_cross_entropy_with_logits(current_outputs, current_labels,
                                                                               reduction='none',
                ...
                all_labels[j].append(labels[:, :, j][current_mask].detach().cpu().numpy())
                all_preds[j].append(outputs[:, :, j][current_mask].detach().cpu().numpy())
...
def validate_dl(model, dataloader, neg_to_pos_ratios,output_size, device,ir_models = None,ci=False,evaluate_net_benefit=False,calibration = False):
    model.eval()
    running_loss = [0.0] * output_size
    all_labels = [[] for _ in range(output_size)]
    all_preds = [[] for _ in range(output_size)]
    ...
    for j in range(output_size):
        if len(all_labels[j]) > 0 and len(all_preds[j]) > 0:
            ...
            if calibration == True and ir_models is not None:
                ...
                prob_true, prob_pred = calibration_curve(all_labels[j], calibrated_preds, n_bins=10,
                                                         strategy='quantile')
            ...
            precision, recall, _ = precision_recall_curve(all_labels[j], calibrated_preds)
            pr_curves.append((precision, recall))


            if ci == True:
                # Compute bootstrapped 95% CI for AUC and AUP
                auc,auc_ci_lower,auc_ci_upper,aup,aup_ci_lower,aup_ci_upper = bootstrap_ci(all_labels[j], calibrated_preds)
                ...
                individual_aup.append(average_precision_score(all_labels[j], calibrated_preds))
            ...
            if evaluate_net_benefit == True:
                ...
                sensitivity, specificity, prevalence = tpr, 1 - fpr, all_labels[j].mean()
    ...
    return running_loss, individual_auc, individual_aup, individual_auc_ci, individual_aup_ci,roc_curves,pr_curves,net_benefit_df,calibration_curves,all_preds,all_labels


def train_and_evaluate_dl(epochs, model, train_dataloader, tune_dataloader, test_dataloader, optimizer, criterion,
                           device, output_size, labels_list, neg_to_pos_ratios, scheduler=None,
                           early_stopping_patience=5, ci=False, my_copy=True, verbose=False, testing=False,calibration=False):
    train_losses = []
    ...
    for epoch in range(epochs):
        ...
        train_loss = train_dl(model, train_dataloader, optimizer, criterion, neg_to_pos_ratios, device)

        # Ccalibrated model
        if calibration == True:
            ir_model_epoch = calibrated_model(model, tune_dataloader, neg_to_pos_ratios, output_size, device)
        else:
            ir_model_epoch = None
        ...
        train_loss_eval, train_auc_eval, train_aup_eval, train_auc_ci_eval, train_aup_ci_eval, _, _, _, _, _, _ = validate_dl(
            model, train_dataloader, neg_to_pos_ratios, output_size, device, ir_models=ir_model_epoch, ci=ci,calibration=False)

        if testing == True:
            val_loss, val_auc, val_aup, val_auc_ci, val_aup_ci, _, _, _, _,  all_preds,all_labels = validate_dl(model, test_dataloader,
                                                                                                neg_to_pos_ratios,
                                                                                                output_size, device,
                                                                                                ir_models=ir_model_epoch,
                                                                                                ci=ci,calibration=True)
        else:
            val_loss, val_auc, val_aup, val_auc_ci, val_aup_ci, _, _, _, _,  all_preds,all_labels = validate_dl(model, tune_dataloader,
                                                                                                neg_to_pos_ratios,
                                                                                                output_size, device,
                                                                                                ir_models=ir_model_epoch,
                                                                                                ci=ci,calibration=False)
        ...
        for i, (loss, auc, aup, auc_ci, aup_ci) in enumerate(zip(val_loss, val_auc, val_aup, val_auc_ci, val_aup_ci)):
        ...
        if verbose == True:
            if ci == False:
                print(f"Epoch {epoch + 1}/{epochs} - Train Loss: {train_loss:.4f}, Mean Val Loss: {mean_val_loss:.4f}")
                ...
                print(f"Epoch {epoch + 1}/{epochs} - Train Loss: {train_loss:.4f}, Mean Val Loss: {mean_val_loss:.4f}")
                for i, (train_auc, train_aup, train_auc_ci, train_aup_ci, val_loss, val_auc, val_aup, val_auc_ci,
                        val_aup_ci) in enumerate(
                        zip(train_aucs, train_aups, train_auc_cis, train_aup_cis, val_losses, val_aucs, val_aups,
                            val_auc_cis, val_aup_cis)):
                    print(
        ...
        if mean_val_loss < best_val_loss:
            print(f"best val loss is {mean_val_loss} and previous loss is {best_val_loss}") if verbose == True else None
    ...
    if output_size == 1:
        ...
        mean_aucs = [np.mean([val_aucs[i][epoch] for i in range(output_size)]) for epoch in range(len(val_aucs[0]))]
        best_epoch = np.argmax(mean_aucs)  # find the epoch with the highest mean AUROC

    #print(f"best epoch is {best_epoch+1}")
    return train_losses, train_aucs, train_aups, val_losses, val_aucs, val_aups, mean_val_losses, train_auc_cis, train_aup_cis, val_auc_cis, val_aup_cis, ir_models, models, all_preds_list,all_labels_list
